...
CREATE TABLE profiles (
  ...
  phone TEXT,
  avatar_url TEXT,
  role TEXT CHECK (role IN ('customer', 'driver', 'dispatcher', 'admin')) DEFAULT 'customer',
  company_name TEXT,
...
-- ============================================
-- DELIVERY EVENTS TABLE (Audit Trail)
-- ============================================
...
-- ============================================
-- TEMPERATURE LOGS TABLE (HIPAA Compliance)
-- ============================================
...
-- ============================================
-- JOB OFFERS TABLE (Driver job matching)
-- ============================================
...
-- ============================================
-- API KEYS TABLE (B2B Integration)
-- ============================================
...
CREATE INDEX idx_drivers_hipaa ON drivers(hipaa_certified) WHERE hipaa_certified = true;
CREATE INDEX idx_drivers_status ON drivers(background_check_status);
...
CREATE INDEX idx_deliveries_booked ON deliveries(booked_at DESC) WHERE booked_at IS NOT NULL;
CREATE INDEX idx_deliveries_hipaa ON deliveries(is_hipaa) WHERE is_hipaa = true;
...
-- Temperature Logs
CREATE INDEX idx_temperature_logs_delivery ON temperature_logs(delivery_id);
CREATE INDEX idx_temperature_logs_time ON temperature_logs(recorded_at DESC);
...
-- Job Offers
CREATE INDEX idx_job_offers_delivery ON job_offers(delivery_id);
CREATE INDEX idx_job_offers_driver ON job_offers(driver_id);
CREATE INDEX idx_job_offers_status ON job_offers(status);
CREATE INDEX idx_job_offers_expires ON job_offers(expires_at) WHERE status = 'pending';
...
-- Drivers: Drivers can manage their own record
...
-- Deliveries: Customers see their own, drivers see assigned, dispatchers see all
...
-- Delivery Events: Viewable by delivery participants
CREATE POLICY "View delivery events" ON delivery_events
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        ...
        OR EXISTS (SELECT 1 FROM drivers WHERE profile_id = auth.uid() AND id = d.driver_id)
        OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'dispatcher'))
      )
    )
  );

-- Job Offers: Drivers see their own offers
...
-- Driver Payouts: Drivers see their own
...
-- Ratings: Viewable by related parties
CREATE POLICY "View ratings" ON ratings
  FOR SELECT USING (
    rated_by = auth.uid()
    OR rated_user = auth.uid()
    OR EXISTS (SELECT 1 FROM drivers WHERE profile_id = auth.uid() AND id = rated_driver)
  );
...
-- Function to expire old job offers
...
-- Insert test company
-- INSERT INTO companies (name, address, city, state, zip, phone, email, hipaa_compliant)
-- VALUES ('Test Medical Lab', '123 Lab Street', 'Los Angeles', 'CA', '90001', '(323) 555-0100', 'lab@example.com', true);

COMMENT ON TABLE deliveries IS 'Main deliveries table - stores all delivery orders';
COMMENT ON TABLE drivers IS 'Driver profiles and capabilities';
COMMENT ON TABLE delivery_events IS 'Audit trail for all delivery status changes - HIPAA compliance';
COMMENT ON TABLE temperature_logs IS 'Temperature monitoring for cold chain compliance';
COMMENT ON TABLE job_offers IS 'Driver job matching - 1-to-1 offer system';
